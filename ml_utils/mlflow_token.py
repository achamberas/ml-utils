# ncsa/mlflow-token: 3d7889d modified
import sys
import time
import os
import urllib
...
def setup_mlflow_environment(tracking_server_url: str, client_secret: str = "") -> None:
    """
    ...
    os.environ["MLFLOW_TRACKING_TOKEN"] = get_mlflow_token(tracking_server_url, client_secret)


def get_keycloak_realm_and_client(mlflow_url: str) -> tuple[str, str]:
    """
...
def get_mlflow_token(tracking_server_url: str, client_secret: str) -> str:
    """
    ...
    """
    realm_url, client_id = get_keycloak_realm_and_client(tracking_server_url)

    r = requests.post(realm_url + "/device", data=[("client_id", client_id), ("client_secret", client_secret)])
    ...
    token_query_url = realm_url.replace("protocol/openid-connect/auth", "protocol/openid-connect/token")
